...
class Engine {
   ...
   * @param {Function} done The callback to invoke after making move
   */
  findAndMakeBestMove(done) {
    let self = this;
    self.send('go', () => {
   ...
   */
  setDepth(depth) {
    this.logger.log('Engine', `Search depth changed to: ${depth}`);
    this.options.depth = depth;
  }

  /**
   * Change the logger debug setting
   * @param {Boolean} bool The flag to set
   */
  setDebug(bool) {
    this.logger.log('Engine', `Debug set to ${bool}`);
    this.logger.options.showDebug = bool;
  }

  /**
   * Change the logger verbose setting
   * @param {Boolean} bool The flag to set
   */
  setVerbose(bool) {
    this.logger.log('Engine', `Verbose set to ${bool}`);
    this.logger.options.showThinking = bool;
  }
}
